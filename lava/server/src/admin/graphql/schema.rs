            ...
            |after, _, first, _| async move {
                ...
                let res = app
                    ...
                    .list(
                        ...
                        es_entity::PaginatedQueryArgs {
                            first,
                            after: after.map(lava_app::withdraw::WithdrawByCreatedAtCursor::from),
                        },
                    )
                    .await?;
                let mut connection = Connection::new(false, res.has_next_page);
                connection
                    ...
                    .extend(res.entities.into_iter().map(|withdraw| {
                        let cursor = WithdrawByCreatedAtCursor::from(&withdraw);
...
impl Mutation {
    ...
    pub async fn credit_facility_create(
        ...
        let credit_facility = app
            .credit_facilities()
            .initiate(sub, customer_id, facility, credit_facility_term_values)
            .await?;
