...
import { userPledgeCollateralServerAction } from "@/app/user/[user-id]/server-actions"

const UserPledgeCollateral = ({
  userId,
  open,
  setOpen,
}: {
  ...
  const handleSubmit = async (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    setResponse(null)
    setLoading(true)
    const amount = amountRef.current ? parseInt(amountRef.current.value, 10) : 0
    ...
    const response = await userPledgeCollateralServerAction({
      userId,
      amount,
      reference,
    })
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>User Pledge Collateral</DialogTitle>
        </DialogHeader>
        {loading ? (
          <p>Loading...</p>
        ) : response ? (
          ...
          <>
            ...
            <div>
              <Label htmlFor="amount">Amount in sats</Label>
              <Input
                ref={amountRef}
                id="amount"
                name="amount"
                min="1"
                type="number"
                placeholder="Please enter amount in sats"
              />
            </div>
            <div>
...
export default UserPledgeCollateral
